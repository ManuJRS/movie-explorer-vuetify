...
const chartSeries = computed(() => {
  return genreStats.value.map(item => item.total)
})

const chartLabels = computed(() => {
  return genreStats.value.map(item => item.label)
})

const totalGenres = computed(() => {
  return genreStats.value.reduce((acc, item) => acc + item.total, 0)
})

const topGenre = computed(() => {
  return genreStats.value[0]?.label ?? 'N/A'
})

const topGenrePercentage = computed(() => {
  if (!genreStats.value.length) return 0

  return Math.round(
    (genreStats.value[0].total / totalGenres.value) * 100
  )
})
  ...
  <div class="glass rounded-xl border border-white/10 bg-white/5 p-6 flex flex-col">
    ...
    <div class="flex-1 flex items-center justify-center relative min-h-[220px]">
      ...
      <div class="absolute flex flex-col items-center">
        <span class="text-2xl font-bold text-white">{{ topGenre }}</span>
        <span class="text-xs text-slate-400">{{ topGenrePercentage }}{{ t('statsFavs.percentTotal') }}</span>
      </div>
      ...
      >
        {{ t('statsFavs.noGenres') }}
      </div>
    </div>

    <div
      v-if="genreStats.length"
      class="grid grid-cols-2 gap-3 mt-6"
    >
      <div
        v-for="(item, index) in genreStats"
        :key="item.label"
