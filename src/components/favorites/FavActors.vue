...
const topActors = computed(() => {
  ...
  for (const movie of favoriteMovies.value) {
    if (!movie.mainActors?.length) continue
...
<template>
  <div class="glass rounded-xl border border-white/10 bg-white/5 p-6 flex flex-col">
    <h3 class="text-lg font-bold text-white mb-6">{{ t('statsFavs.actorsTitle') }}</h3>

    <div v-if="topActors.length" class="space-y-4">
      <div
        v-for="(item, index) in topActors"
        :key="item.label"
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="text-xs text-slate-500 uppercase font-medium">
            {{ t('statsFavs.actor') }} • {{ item.total }} {{ item.total === 1 ? t('statsFavs.movie_one') : t('statsFavs.movie_other') }}
          </p>
        </div>
      </div>
    </div>
